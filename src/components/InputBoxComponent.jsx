...
function InputBoxComponent({
  ...
  return (
    <div className="relative w-full max-w-sm">
      ...
      {password ? (
        <div className="flex flex-row justify-center border border-gray-300 rounded-lg px-4 py-5 pr-12 focus:ring-2 focus:ring-teal-500">
          <input
          ...
          >
            {showPass ? (
              <FiEyeOff className="h-5 w-5" color="black" />
            ) : (
              <FiEye className="h-5 w-5" color="black" />
            )}
          </button>
        </div>
      ) : (
